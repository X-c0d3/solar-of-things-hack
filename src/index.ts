...
async function poll() {
  try {
    ...
    console.log('\n🔋  BATTERY');
    console.log(
      `   ${V(out.battery_voltage)}V  ${V(out.battery_current)}A  ${V(out.battery_power)}kW  SOC ${V(out.battery_soc)}%  SOH ${V(out.battery_soh)}%`,
    );
    ...
    console.log('\n🌡️   TEMP');
    console.log(
      `   heatsink ${V(out.temp_heatsink)}  radiator ${V(out.temp_radiator)}  PV ${V(out.temp_pv)}  OP ${V(out.temp_op)}  env ${V(out.temp_env)}  cell ${V(out.temp_cell_avg)} °C`,
    );
